...
export class TransactionsRepository {
    ...
    try {
      ...
      if (!transaction) {
        throw new HttpException(
          `Transaction  for userid: ${userId} not found!!!`,
  ...
  async getTransactionByCurrencyAndUserId(
    userId: string,
    currencyCode: string,
  ): Promise<any> {
    try {
      const transaction = await this.transaction.find({
        $and: [{ user_id: userId, currency: currencyCode }],
      });
      if (!transaction) {
        throw new HttpException(
          `Transaction  for userid: ${userId} and currency: ${currencyCode}  not found!!!`,
          HttpStatus.NOT_FOUND,
        );
      }
  ...
  async getTransactionById(id: string): Promise<Transaction> {
    try {
      ...
      if (!transaction) {
        throw new HttpException(
          `Transaction  for userid: ${id} not found!!!`,
          HttpStatus.NOT_FOUND,
        );
      }
      return transaction;
  ...
  async updateTransaction(id: string, data: any): Promise<Transaction> {
    try {
      const transaction = await this.transaction.findByIdAndUpdate(id, data, {
        new: true,
      });
      if (!transaction) {
        throw new HttpException(
