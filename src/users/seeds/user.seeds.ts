...
export class UserSeed{
    ...
    async create(){
     const    isUserExit = await this.usersRepository.findUserByEmail('josiane@rssb.rw');
     if(isUserExit){
        ...
        const user = await this.usersRepository.insertOne({
                ...
                email:'josiane@rssb.rw',//mmaezanirarutza@rssb.rw
        ...
        console.log({email:user.email,password:'rssb123',role:user.role,createdAt:user.createdAt,status:user.status});
     }
    }
}