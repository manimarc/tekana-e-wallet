...
export class UsersService {
  ...
  async update(
    ...
    const { names, email, password, phone, status } = updateUserDto;
  ...
  async remove(id: string): Promise<UserResponse> {
    ...
    if (!deletedUser) {
      throw new BadRequestException(
        `There is a problem deleting user with _id: ${id}`,
      );
    }
